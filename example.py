# Note on the consequences of changing en_core_web_md to en_core_web_sm:

# Running the example.py with en_core_web_sm instead of en_core_web_md generates the warning message:

# UserWarning: [W007] The model you're using has no word vectors loaded, so the result of the Doc.similarity method will
# be based on the tagger, parser and NER, which may not give useful similarity judgements. This may happen if you're
# using one of the small models, e.g. `en_core_web_sm`, which don't ship with word vectors and only use
# context-sensitive tensors. You can always add your own word vectors, or use one of the larger models instead if
# available.

# And when you compare the similarity rates themselves, it's clear that on the small model, the similarity rates
# drop significantly (from averages of 0.9+ to 0.5+). I think this is because the small model has less to make
# comparisons with. The bigger the model, the more similarities can be found.
